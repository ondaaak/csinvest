...
export default function CharmsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const url = q ? `${API_BASE}/search/items?q=${encodeURIComponent(q)}&item_type=charm` : `${API_BASE}/items?item_type=charm`;
        const res = await fetch(url);
        if (!res.ok) throw new Error('Failed to load charms');
  ...
  const doRefreshPrices = async () => {
    try {
      setRefreshing(true);
      await fetch(`${API_BASE}/refresh-items?item_type=charm`, { method: 'POST' });
      const url = query ? `${API_BASE}/search/items?q=${encodeURIComponent(query)}&item_type=charm` : `${API_BASE}/items?item_type=charm`;
      const res = await fetch(url);
      const data = await res.json();
  ...
  const getRarityColor = (rarity) => {
    ...
    if (r.includes('mil-spec') || r.includes('high grade')) return { bg: 'rgba(75, 105, 255, 0.25)', color: '#4b69ff' };
  ...
  return (
    <div className="dashboard-container">
      <div style={{ display:'flex', gap:12, alignItems:'center', marginBottom:12 }}>
        ...
        <h2 style={{ margin:0, flex:1, paddingLeft:'14%' }}>Charms</h2>
      ...
      <div className="categories-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        {sortedItems.map(it => (
          ...
          >
            {getCharmImage(it.slug) ? (
              <img src={getCharmImage(it.slug)} alt={it.name} className="category-img" />
            ) : (
        ...
        {(!loading && items.length === 0) && (
          <div style={{ textAlign: 'center', width: '100%', color: '#6b7280' }}>No charms found.</div>
        )}
      </div>
    </div>
  );
